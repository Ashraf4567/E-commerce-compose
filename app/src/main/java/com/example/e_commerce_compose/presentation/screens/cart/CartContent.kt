...
fun CartScreenContent(
    ...
    onBackClicked: () -> Unit,
    onCheckoutClicked: (cartId: String) -> Unit
) {
    ...
    LaunchedEffect(key1 = true){
        cartViewModel.effect.collect{effect ->
            when(effect){
                ...
                CartEffects.NavigateToCheckout -> onCheckoutClicked(state.cart.id?:"")
                    ...
                    state.cart.products.let {
                        items(
                            ...
                            CartItem(
                                ...
                                isLoading = state.specificProductLoading.first == state.cart.products!![index]?.id && state.specificProductLoading.second
                            )
                        }
                    }
                }
